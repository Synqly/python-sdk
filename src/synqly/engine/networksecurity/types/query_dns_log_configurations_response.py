# This file was auto-generated by Fern from our API Definition.

from __future__ import annotations

import typing

import pydantic
from ...core.pydantic_utilities import IS_PYDANTIC_V2, update_forward_refs
from ...engine.types.api_query_response import ApiQueryResponse
from .dns_log_configuration import DnsLogConfiguration


class QueryDnsLogConfigurationsResponse(ApiQueryResponse):
    result: typing.List[DnsLogConfiguration] = pydantic.Field()
    """
    List of DNS log configurations matching the query.
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow")  # type: ignore # Pydantic v2
    else:

        class Config:
            smart_union = True
            extra = pydantic.Extra.allow


update_forward_refs(QueryDnsLogConfigurationsResponse)
